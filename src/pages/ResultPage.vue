<template>
  {{ calculate }}               <!--вызов основной функции подсчета долгов-->
...
<v-card class="mx-auto overflow-y-auto v-card resultcard" max-width="500" max-height="495">
  <div class="debt" v-for="(person, idx) in storage.persons" :key="idx">
    ...
    <p v-if="checkDebt(storage.persons[idx].debt)">Да никому он не должен, живет как хочет</p>  <!--вызов функции проверки наличия долгов, если их нет, то показ соответствующего сообщения-->
    <p v-else v-for="(debt,index) in storage.persons[idx].debt" 
    :key="index" v-show="debt"> {{storage.persons[index].name}} : <b>{{ debt }}</b></p>  <!--отображение долга человека другим людям, если он кому то ничего не должен, то это отображаться не будет-->
  </div>
</v-card>
</template>
