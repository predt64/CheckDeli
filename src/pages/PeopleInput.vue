<template>
    <v-container>
      <action-button name="Добавить человека" @click="addPerson()" />                            <!-- Кнопка добавления человека-->

      <v-card class="mx-auto overflow-y-auto v-card" max-width="500" max-height="495">
        <input-people @removePerson="removePerson"></input-people>                                <!--Компонент ввода имени человека и его удаление-->
      </v-card>
      <v-sheet max-width="300" class="mx-auto sheet">
        <action-button name="Перейти дальше" @click="checkNames(), changeState(), redirect()" /> <!--Кнопка перехода на следующую страницу, перед этим вызывает функции для проверки корректности введенных данных-->
